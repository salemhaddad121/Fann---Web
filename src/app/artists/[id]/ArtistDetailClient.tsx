...
function Content({ id, initialArtist }: { id: string; initialArtist: ArtistDetail | null }) {
  const { user } = useAuth();
  const router = useRouter();
  const [artist, setArtist] = useState<ArtistDetail | null>(initialArtist);
  const [error, setError] = useState<string | null>(null);
  ...
  // The server hands down the profile for visitors with no session, so
  // there is nothing to fetch and nothing to wait for — the profile is in
  // the HTML. A signed-in visitor gets no server copy on purpose: the API
  // returns more of the record to a paying viewer than it does to the
  // anonymous server render, and painting the masked version first would
  // show a subscriber "Karim N." before correcting itself.
  useEffect(() => {
...
export function ArtistDetailClient({
  ...
  id: string;
  initialArtist: ArtistDetail | null;
}) {
  ...
  //
  // When the server has already resolved the profile there is no session to
  // wait for, so the guest frame renders immediately rather than blanking:
  // returning null here is what previously left a crawler with an empty
  // document on the most indexable page Fann has.
  if (isLoading && !initialArtist) return null;
