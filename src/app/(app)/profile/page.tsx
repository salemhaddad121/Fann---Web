...
function ArtistOwnProfile({ userId, status }: { userId: string; status: UserStatus }) {
  ...
  return <ArtistProfileView artist={artist} isOwnProfile accountStatus={status} />;
}
...
function PlannerOwnProfile({ userId, status }: { userId: string; status: UserStatus }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const profile = await getMyPlannerProfile();
        if (cancelled) return;
        setPlanner(profile);
      } catch {
        if (!cancelled) setError("Couldn't load your profile. Try refreshing.");
  ...
  return <PlannerProfileView planner={planner} isOwnProfile accountStatus={status} />;
}
